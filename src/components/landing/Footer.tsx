...
// ─── OUTSIDE the graph: cyan only ────────────────────────────────────────────
const CYN_BRIGHT = new THREE.Color(0x06b6d4); // cage, UI accents (unused in chart)
const CYN_MID = new THREE.Color(0x0891b2); // buttons, borders
const GREY_AXIS = new THREE.Color(0x888899); // axes, ticks
const GREY_GRID = new THREE.Color(0x444455); // gridlines
const GREY_CAGE = new THREE.Color(0xaaaabb); // cage wireframe

// ─── INSIDE the graph: professional Tableau/Plotly-style multi-color ──────────
// Pie slices — 5 distinct hues
const PIE_C = [
  new THREE.Color(0x4e79a7), // steel blue
  new THREE.Color(0xf28e2b), // warm orange
  new THREE.Color(0xe15759), // coral red
  new THREE.Color(0x76b7b2), // teal
  new THREE.Color(0x59a14f), // forest green
];
// Error line chart
const ERR_LINE = new THREE.Color(0x4e79a7); // steel blue main line
const ERR_BAND = new THREE.Color(0x4e79a7); // matching band fill
const ERR_BARS = new THREE.Color(0xf28e2b); // orange error bars
const ERR_DOTS = new THREE.Color(0xe15759); // red data point dots
// Contour: Viridis-inspired colorscale stops
...
// ─── Chart sequence timings ───────────────────────────────────────────────────
const HOLD_FRAMES = 220; // frames each chart stays visible
const MORPH_FRAMES = 100; // frames to morph between charts
type ChartMode = "pie" | "errorline" | "contour";
const CYCLE: ChartMode[] = ["pie", "errorline", "contour"];

// ─── Data ─────────────────────────────────────────────────────────────────────
const PIE_VALS = [0.32, 0.22, 0.18, 0.15, 0.13]; // must sum to 1
...
function ThreeCubeFooter() {
  ...
  useEffect((): (() => void) => {
    ...
    // ── Helper: set mesh opacity + visibility ──
    ...
    // Build pie slices as extruded arcs
    const pieSlices: THREE.Mesh[] = [];
    let cumAngle = 0;
    PIE_VALS.forEach((val, i) => {
      ...
      pieGroup.add(
        new THREE.LineSegments(new THREE.EdgesGeometry(geo), edgeMat),
      );
    });

    // Pie labels — small tick lines
    cumAngle = 0;
    ...
    // Error band (filled area between hi/lo)
    ...
    errGroup.add(bandMesh);

    // Error bar verticals at each point
    LINE_X.forEach((x, i) => {
    ...
    // Filled cells — colored by z value
    for (let cy = 0; cy < CONT_H - 1; cy++) {
      for (let cx = 0; cx < CONT_W - 1; cx++) {
        const z00 = contourZ(cx, cy);
        const z10 = contourZ(cx + 1, cy);
        const z01 = contourZ(cx, cy + 1);
        const z11 = contourZ(cx + 1, cy + 1);
        const zAvg = (z00 + z10 + z01 + z11) / 4;

        // Map z [0..1] → Viridis-style multicolor
        const col = new THREE.Color();
        let ci = 0;
        while (ci < CONT_STOPS.length - 2 && zAvg > CONT_STOPS[ci + 1][0]) ci++;
        const [t0, c0] = CONT_STOPS[ci];
        const [t1, c1] = CONT_STOPS[ci + 1];
        const localT = (zAvg - t0) / (t1 - t0);
        col.lerpColors(c0, c1, Math.max(0, Math.min(1, localT)));

        const cellW = CW / (CONT_W - 1);
        const cellH = CH / (CONT_H - 1);
    ...
    LEVELS.forEach((level, li) => {
      ...
      for (let cy = 0; cy < CONT_H - 1; cy++) {
        for (let cx = 0; cx < CONT_W - 1; cx++) {
          ...
          // simple linear interpolation on each edge
          const edges = [
            [0, 1],
            [1, 2],
            [2, 3],
      ...
      if (points.length > 0) {
        ...
        contGroup.add(new THREE.LineSegments(geo, isoMat));
      }
    });

    // ── Visibility helpers ──
    const showPie = (op: number) => {
      setOp(pieGroup, op);
    };
    const showErr = (op: number) => {
      setOp(errGroup, op);
    };
    const showContour = (op: number) => {
      setOp(contGroup, op);
    };
...
// ─── Closing words (word-by-word like hero) ───────────────────────────────────
const CLOSING_WORDS = "The last chart tool you'll ever need.".split(" ");

export default function HeroFooterCTA() {
  ...
  return (
    <>
      <style>{`
        .fx-root { background: #111; color: #fff; font-family: monospace; }
        ...
        /* entry animations */
        .fx-root [data-anim]                      { opacity: 0; }
        .fx-root.fx-ready [data-anim="nav-strip"] { animation: fxFD .45s ease  0.0s forwards; }
        .fx-root.fx-ready [data-anim="bar"]       { animation: fxFD .5s  ease  0.0s forwards; }
        .fx-root.fx-ready [data-anim="cube"]      { animation: fxSI .85s cubic-bezier(.22,1,.36,1) .3s forwards; }
        .fx-root.fx-ready [data-anim="sub"]       { animation: fxFU .6s  ease  0.72s forwards; }
        .fx-root.fx-ready [data-anim="stats"]     { animation: fxFU .6s  ease  0.80s forwards; }
        .fx-root.fx-ready [data-anim="cta"]       { animation: fxFU .6s  ease  0.90s forwards; }
        .fx-root.fx-ready [data-anim="legal"]     { animation: fxFU .5s  ease  1.0s  forwards; }

        ${CLOSING_WORDS.map(
          (_, i) => `
          .fx-root.fx-ready [data-anim="word-${i}"] {
            animation: fxW .55s cubic-bezier(.22,1,.36,1) ${0.08 + i * 0.08}s forwards;
          }
        `,
        ).join("")}

        @keyframes fxFD { from{opacity:0;transform:translateY(-16px)} to{opacity:1;transform:translateY(0)} }
        @keyframes fxFU { from{opacity:0;transform:translateY(16px)}  to{opacity:1;transform:translateY(0)} }
        @keyframes fxW  { from{opacity:0;transform:translateY(22px)}  to{opacity:1;transform:translateY(0)} }
        @keyframes fxSI { from{opacity:0;transform:scale(0.93) translateY(20px)} to{opacity:1;transform:scale(1) translateY(0)} }
        @keyframes fxBlink { 0%,100%{opacity:1} 50%{opacity:0} }

        .fx-cursor {
          display: inline-block; width: 3px; height: 0.85em;
          background: #0891b2; margin-left: 3px; vertical-align: text-bottom;
          animation: fxBlink 1.1s step-end infinite;
        }
        .fx-cta-btn {
        ...
        .fx-cta-btn:hover { filter: brightness(1.25); transform: translateY(-1px); }
        ...
        .fx-cta-ghost:hover { background: #fff; color: #111; }

        .fx-stat-val {
          font-size: 2.2rem; font-weight: 800;
          letter-spacing: -0.04em; line-height: 1;
        }
        .fx-stat-label { font-size: 10px; letter-spacing: 0.18em; color: #888899; margin-top: 4px; }

        .fx-footer-link {
          font-size: 10px; color: #888899; letter-spacing: 0.18em;
          text-decoration: none; transition: color .15s;
        }
        .fx-footer-link:hover { color: #06b6d4; }
      `}</style>

      <div className={`fx-root${ready ? " fx-ready" : ""}`}>
       
        {/* ── BODY: cube LEFT, text RIGHT (flipped from hero) ── */}
            ...
            <div style={{ position: "relative", zIndex: 1 }}>
              ...
              >
                {[
                  ...
                  <div key={s.label}>
                    <div className="fx-stat-val" style={{ color: s.color }}>
                      {s.override ?? (
              ...
              >
                <button className="fx-cta-btn">Install Now</button>
                <button className="fx-cta-ghost">View Docs</button>
              </div>
        ...
        >
          <span
            style={{ fontSize: 10, color: "#444455", letterSpacing: "0.18em" }}
          >
            © 2025 GRAPHIX · ALL RIGHTS RESERVED
          </span>
          <div style={{ display: "flex", gap: 24 }}>
            {["PRIVACY", "TERMS", "STATUS", "TWITTER"].map((link) => (
              <a key={link} href="#" className="fx-footer-link">
                {link}
              </a>
            ))}
          </div>
